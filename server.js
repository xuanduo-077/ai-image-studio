...
要求：
1. characters 最多 5 个主要角色；appearance 为 50-90 字的详细外貌设定（性别与年龄段、发型发色、五官特征、体型、标志性服装与配饰），将用于生成角色三视图，必须具体可画，不要写性格
2. scenes 最多 8 个故事主要发生场景；description 为 60-120 字的场景环境设定（空间布局、建筑与陈设特征、时间与光线氛围、季节天气），将用于生成场景设定图，画面中不会出现人物
...
3. videoPrompt：40-80 字的完整画面描述 = 景别 + 出场角色及其动作神态（关键外貌特征要提及）+ 环境要点 + 镜头运动；这是生成视频的唯一画面描述，必须完整自洽
